...
if __name__ == "__main__":
    ...
    def build_conditionally(algorithm, tree_path, entries_path, logfile, beta=0.5, limit=None):
        if tree_path.exists():
    ...
    with (OUTPUT_PATH / "tree_variants.log").open("w") as logfile:
        ...
        def build_beta_modes():
            ...
            for mode in ["increasing", "decreasing"]:
                ...
                for dataset, entries, data_path in datasets:
                    for algorithm in algorithms:
                        build_conditionally(algorithm, tree_path=tree_path(dataset, algorithm, mode),
        ...
        def build_bloom_filters():
            ...
            for dataset, entries, data_path in datasets:
                for algorithm in algorithms:
                    build_conditionally(algorithm, tree_path=tree_path(dataset, algorithm),
                                        entries_path=data_path, logfile=logfile, limit=entries)
