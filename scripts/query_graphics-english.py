...
def load_results():
    with (RESULT_PATH / "queries_beta_09.json").open("r") as file:
...
names = {
    "small": "Small Queries",  # "Kleine Anfragen",
    "large": "Large Queries",  # "Große Anfragen",
    "sequenced": "Sequenced Queries"  # "Sequentielle Anfragen"
}
...
def key_ylabel(key):
    if key == "duration":
        return "Time (avg., in sec.)"  # "Durchschnittliche Dauer (s)"
    if key == "total_io":
        return "I/Os (avg.)"  # "Durchschnittliche I/Os"
...
def plot_main_algorithms(output_path):
    ...
    datasets = [
        (("GeoLife", "geolife"), [
            ("Hilbert", "geolife-hilbert-beta-0.9"),
            ("STR-LF", "geolife-str-lf-beta-0.9"),
            ("QuickLoad ($\\beta = 0.9$)", "geolife-quickload-beta-0.9"),
            ("OBO ($\\beta = 0.9$)", "geolife-obo-beta-0.9")
        ]),
        (("OSM", "osm"), [
            ("Hilbert", "osm-hilbert-beta-0.9"),
            ("STR-LF", "osm-str-lf-beta-0.9"),
            ("QuickLoad ($\\beta = 0.9$)", "osm-quickload-beta-0.9"),
            ("OBO ($\\beta = 0.9$)", "osm-obo-beta-0.9")
        ]),
...
def plot_fanout(output_path):
    ...
    ax1.set_title("Query Cost (I/Os)")
    ax1.set_xticks(x)
    ax1.set_xlabel("Fan-Out")
    ax1.set_ylabel("I/Os (avg.)")
    ax1.legend(loc="best")
    ...
    ax2.plot(x, rtree_size, marker="o", linestyle="dashed", label="R-tree")
    ax2.plot(x, index_size, marker="^", linestyle="dashed", label="inv. Index")

    ax2.set_title("Size of Data Structure")
    ax2.set_xticks(x)
...
plot_main_algorithms(RESULT_PATH / "english" / "query_beta_0.9.pdf")
#plot_beta_values(RESULT_PATH / "english/query_beta_values.pdf")
#plot_beta_strategies(RESULT_PATH / "english/query_beta_strategies.pdf")
#plot_str_variants(RESULT_PATH / "english/query_str_variants.pdf")
#plot_bloom_filters(RESULT_PATH / "english/query_bloom_filters.pdf")
#plot_fanout(RESULT_PATH / "english/query_fanout.pdf")
